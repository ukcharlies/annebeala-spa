    ...
    <>
      <section className="section-shell pt-8 md:pt-12">
        <div className="relative overflow-hidden rounded-[2rem] border border-brand-olive/30 bg-brand-charcoal">
          <div className="absolute inset-0">
            <video autoPlay loop muted playsInline preload="metadata" className="h-full w-full object-cover opacity-30">
          ...
          <div className="relative grid gap-8 p-6 md:p-10 lg:grid-cols-[1.05fr,0.95fr] lg:gap-12 lg:p-12">
            <div className="pt-3 md:pt-8">
              <p className="text-xs uppercase tracking-[0.26em] text-brand-sage">Annebeala Spa • Lagos</p>
              ...
              <p className="mt-6 max-w-2xl text-base leading-7 text-brand-ivory/80 md:text-lg">
                Luxury massages, facials, and body rituals curated to help you unwind, glow, and
                feel your best in both body and mind.
              </p>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                <article className="rounded-xl border border-brand-ivory/20 bg-brand-ivory/10 p-4 backdrop-blur">
                  <p className="text-xs uppercase tracking-[0.18em] text-brand-sage">Now Serving</p>
                  <p className="mt-2 text-xl text-brand-ivory">Ikeja + VI</p>
                </article>
                <article className="rounded-xl border border-brand-ivory/20 bg-brand-ivory/10 p-4 backdrop-blur">
                  <p className="text-xs uppercase tracking-[0.18em] text-brand-sage">Instagram</p>
                  <p className="mt-2 text-xl text-brand-ivory">@annebeala_spa</p>
                </article>
              </div>
            </div>
            ...
            <div className="grid min-h-[26rem] grid-cols-12 grid-rows-12 gap-3 sm:min-h-[34rem]">
              ...
              <div className="col-span-4 row-span-4 rounded-2xl border border-brand-sage/45 bg-brand-ivory/10 p-4 backdrop-blur">
                <p className="text-xs uppercase tracking-[0.15em] text-brand-sage">Open Daily</p>
                <p className="mt-3 text-2xl text-brand-ivory">9:30am - 7:00pm</p>
                <p className="mt-2 text-xs leading-6 text-brand-ivory/75">Sunday from 12:00pm at both branches.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="mt-16 bg-brand-charcoal py-16 text-brand-ivory">
        <div className="section-shell">
          <div className="max-w-4xl">
            <p className="text-xs uppercase tracking-[0.22em] text-brand-sage">Our Services</p>
            <h2 className="mt-3 text-5xl">Luxury Treatments, Clearly Structured</h2>
            <p className="mt-4 text-base leading-7 text-brand-ivory/80">
              We combine therapeutic spa methods with premium hospitality. Explore our most requested
              services with duration and starting rates.
            </p>
          </div>
      ...
      <section className="section-shell mt-16">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h2 className="mt-3 text-4xl text-brand-charcoal md:text-5xl">Wellness Plans That Sell Themselves</h2>
          </div>
        ...
        <div className="rounded-3xl border border-brand-olive/25 bg-brand-charcoal p-6 text-brand-ivory md:p-10">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div className="max-w-2xl">
              <p className="text-xs uppercase tracking-[0.2em] text-brand-sage">Instagram Reels</p>
              <h2 className="mt-3 text-5xl">Inside Annebeala Spa</h2>
              <p className="mt-4 text-sm leading-7 text-brand-ivory/80">
                See real treatment sessions, behind-the-scenes clips, and client results from our
                Instagram reels.
              </p>
            </div>
            <Link
              ...
              <article key={reel.title} className="overflow-hidden rounded-2xl border border-brand-sage/30 bg-brand-charcoal/70">
                <div className="relative aspect-[9/12]">
                  {reel.type === "video" ? (
                    ...
                    <Image src={reel.src} alt={reel.title} fill sizes="(max-width: 1024px) 50vw, 25vw" className="object-cover" />
                  )}
                ...
                <div className="p-4">
                  ...
                  <Link href={reel.url} target="_blank" rel="noreferrer" className="mt-3 inline-block text-xs uppercase tracking-[0.14em] text-brand-sage underline underline-offset-4">
                    Watch on Instagram
                  </Link>
                </div>
              </article>
      ...
      <section className="section-shell mt-16">
        <div className="rounded-3xl border border-brand-olive/25 bg-brand-ivory p-7 md:p-10">
          <div className="max-w-3xl">
            <p className="text-xs uppercase tracking-[0.2em] text-brand-olive">What Clients Say</p>
            ...
            <p className="mt-4 text-sm leading-7 text-brand-charcoal/75">
              Consistent results, warm hospitality, and visible improvements keep our guests coming
              back.
            </p>
          </div>
            ...
            {homeReviewShowcase.map((review) => (
              <article key={review.name} className="rounded-2xl border border-brand-olive/30 bg-white p-6 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-xl text-brand-charcoal">{review.name}</h3>
                ...
                <p className="mt-3 text-sm leading-7 text-brand-charcoal/80">“{review.quote}”</p>
              </article>
            ))}
          </div>
      ...
      <section className="section-shell mt-16 pb-2">
        <div className="rounded-3xl border border-brand-olive/25 bg-brand-charcoal p-7 text-brand-ivory md:p-10">
          <p className="text-xs uppercase tracking-[0.2em] text-brand-sage">Why Choose Annebeala</p>
          <h2 className="mt-3 text-5xl">Professional Care, Premium Experience</h2>
      ...
      <section className="section-shell mt-16">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-brand-olive">Visit Us</p>
            <h2 className="mt-3 text-4xl text-brand-charcoal">Two Branches, One Premium Standard</h2>
          </div>
